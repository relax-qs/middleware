...
import com.lark.middleware.util.ByteUtil.LexicographicalComparerHolder.UnsafeComparer;
...
public class ByteUtil {
    ...
    public static int toIntUnsafe(byte[] bytes, int offset) {
        if (UnsafeComparer.littleEndian) {
            return Integer.reverseBytes(UnsafeComparer.theUnsafe.getInt(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET));
        } else {
    ...
    public static short toShortUnsafe(byte[] bytes, int offset) {
        if (UnsafeComparer.littleEndian) {
            return Short.reverseBytes(UnsafeComparer.theUnsafe.getShort(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET));
        } else {
    ...
    public static long toLongUnsafe(byte[] bytes, int offset) {
        if (UnsafeComparer.littleEndian) {
            return Long.reverseBytes(UnsafeComparer.theUnsafe.getLong(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET));
        } else {
    ...
    public static byte[][] toBinaryByteArrays(final String[] t) {
        byte[][] result = new byte[t.length][];
        for (int i = 0; i < t.length; i++) {
            result[i] = ByteUtil.toBytesBinary(t[i]);
        }
        return result;
    }
